...
async function main() {
  ...
  const reasonText = 'manual cleanup of stale pending_start order after synchronous start timeout';
  const failureSource = 'manual_cleanup';
  const failureMessage = 'manual cleanup of stale pending_start order';
  ...
  try {
    ...
    const result = await db.withTransaction(async (client) => {
      ...
      return {
        ...
        orderNo: row.orderNo,
        sessionId: row.sessionId,
        previousSessionStatus: row.sessionStatus,
        settled,
      };
    });
